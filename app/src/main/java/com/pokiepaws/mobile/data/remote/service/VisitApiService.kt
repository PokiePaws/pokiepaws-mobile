...
interface VisitApiService {
    ...
    suspend fun getVisitsByAnimal(@Path("animalId") animalId: Long): List<VisitResponse>

    @POST("api/visits")
    suspend fun createVisit(@Body request: CreateVisitRequest): VisitResponse

    @GET("api/visits/{id}")
    suspend fun getVisitById(@Path("id") visitId: Long): VisitResponse
    @PATCH("api/visits/{id}/cancel")
    suspend fun cancelVisit(@Path("id") visitId: Long): VisitResponse
}